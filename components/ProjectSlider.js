                ...
                <div class="lg:w-4/5 lg:mx-auto">
                  <ProjectPreview>
                    {project.node.acf.gallery?.length > 1 ? (
                      ...
                      <img src={ project.node.acf.gallery ? project.node.acf.gallery[0].sourceUrl : '' } className="rounded-lg w-full mx-auto"/>
                    )}
                  </ProjectPreview>
                </div>

                <div class="lg:w-4/5 lg:mx-auto flex justify-center flex-col relative z-30 pt-3 px-3">
                  <h2 className="text-teal-300 text-2xl sm:text-4xl md:text-5xl font-bold mb-3">{ project.node.title }</h2>
                  <p class="text-md sm:text-xl sm:mb-3 font-bold italic text-fuchsia-200">Built with { project.node.acf.type } in { new Date(project.node.date).getFullYear() }</p>
