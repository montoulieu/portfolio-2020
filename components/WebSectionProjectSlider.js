...
function WebSectionProjectSlider(props) {
            ...
            <div className="slide-wrapper columns-2 mx-auto">
              <div className="grid-container">
                <div class="lg:w-4/5 lg:mx-auto">
                  <ProjectPreview>
                    {project.node.acf.gallery.length > 0 ? (
                      <Fade {...fadeProperties}>
                        {project.node.acf.gallery.map((image) =>
                          <div className="each-fade">
                            <div className="image-container">
                              <img src={image.sourceUrl} className="rounded w-full" />
                            </div>
                          </div>
                        )}
                      </Fade>
                    ): (
                      <img src={ project.node.acf.gallery[0].sourceUrl } className="rounded-lg w-full"/>
                    )}
                  </ProjectPreview>
                </div>

                <div class="lg:w-4/5 lg:mx-auto flex justify-center flex-col relative z-30">
                  <h2 className="text-teal-300 text-4xl md:text-5xl font-bold mb-3">{ project.node.title }</h2>
                  <p class="text-xl mb-3 italic text-fuchsia-200">Built with { project.node.acf.type } in { new Date(project.node.date).getFullYear() }</p>
                  <div class="mb-3 text-blue-200" dangerouslySetInnerHTML={{ __html: project.node.acf.description }}>
                  </div>
                  {project.repoUrl &&
                    <div>
